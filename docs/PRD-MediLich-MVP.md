...
### 3.2 Hành trình đau đớn hiện tại (AS-IS)
...
### 3.3 Hành trình với MediLịch (TO-BE)
...
|---|---|---|---|
| F1 | **Scan đơn thuốc** | Upload ảnh → OpenAI Vision → JSON tự động | `openai-parse.js` |
| F2 | **Xác nhận & chỉnh sửa** | Hiển thị danh sách thuốc, cho phép sửa trước khi lưu | `app.js:renderReview()` |
| F3 | **Lịch nhắc tự động** | Tạo lịch từ đơn, hiển thị nhắc tiếp theo, "Đã uống" | `schedule.js` |
| F4 | **Thẻ thuốc** | Tra công dụng AI + citations nguồn (DrugBank, BYT...) | `openai-drug.js` |
...
| # | Tính năng | Mô tả | File |
|---|---|---|---|
| F6 | **Lịch tháng** | Xem lịch uống theo tháng, sync đơn ↔ lịch nhắc | `calendar-ui.js` |
| F7 | **Tìm nhà thuốc gần** | Dùng OpenStreetMap, không cần API trả phí | `nearby-places.js` |
| F8 | **Giả lập thông báo** | Mô phỏng push notification trong demo | `app.js:showNotif()` |
...
| **API Key** | `OPENAI_API_KEY` trong `.env` | `.env.example` có hướng dẫn |
